...
export const useFormStore = defineStore("form", () => {
  ...
  if (saved) {
    try {
      ...
      if (saved.profilingData)
        Object.assign(profilingData.value, saved.profilingData);
      if (saved.amvatProfile)
        Object.assign(amvatProfile.value, saved.amvatProfile);
  ...
  const eligibility = computed(() => {
    ...
    if (d.birth_date) {
      ...
      if (parts.length === 3) {
        ...
        if (
          ...
          (today.getMonth() === bmonth && today.getDate() < bday)
        )
          age--;
      }
    }
    ...
    if (d.has_child === "No") {
      reasons.push(
        "Must have at least one living child (currently pregnant)",
      );
    } else if (
      d.has_child === "Yes" &&
      parseInt(d.number_children || "0") < 1
    ) {
      reasons.push("Must have at least one living child");
    }

    if (!age || age <= 0 || age > 19)
      reasons.push(
        `Age must be 19 years old or below (currently ${age > 0 ? age : "unknown"})`,
      );
  ...
  function setProfilingStep(n) {
    profilingStep.value = n;
  }
  ...
  function prefillAmvatFromProfiling() {
    ...
    const occupationMap = {
      ...
      "Service worker/shop & market worker":
        "Service worker/shop & market worker",
      "Farmers, forestry worker & fishermen":
        "Farmers, forestry worker & fishermen",
    ...
    const isCustomOccupation =
      d.occupation && !standardOccupations.includes(d.occupation);
    const isCustomReligion =
      d.religion && !standardReligions.includes(d.religion);
    ...
    Object.assign(amvatProfile.value, {
      ...
      barangay: d.barangay,
      street_sitio: [d.street_address, d.sitio_purok]
        .filter(Boolean)
        .join(", "),
      contact: d.contact,
      civilStatus: d.civil_status,
      hasChild:
        d.has_child === "Yes" ? "Oo" : d.has_child === "No" ? "Hindi" : "",
